...
def tokenize_et_style(text):
    # Split text into tokens. Hyphenated words are split.
    raw_words = text.replace('\n', ' ').split(' ')
    tokens = []
    for w in raw_words:
        if w.strip() == '':
            continue
        # Split on hyphen but keep the hyphen in the first part, just like ET
...
# We'll test both normal and ET-style tokenization
for tok_style in ["normal", "et"]:
    for max_w in range(1200, 1600, 10):
        for cap_scale in [0.8, 0.9, 1.0, 1.1]:
            for digit_scale in [0.8, 0.9, 1.0, 1.1]:
                for space_w in [24, 26, 28, 30]:
                    for b_mult in [1.2, 1.3, 1.4]:
                        for f_b_mult in [1.2, 1.4, 1.6]:
                            ...
                            for s in samples:
                                if tok_style == "et":
                                    tokens = tokenize_et_style(s["text"])
                                else:
                                    tokens = tokenize_normal(s["text"])
                                    
                                calc = calculate_lines(tokens, s["bold_words"], max_w, widths, b_mult, f_b_mult)
                                results.append(calc)
                                if calc == s["expected"]:
                                    matches += 1
                            ...
                            if matches > best_matches:
                                ...
                                print(f"New best: {matches}/5 | params: {best_params}")
                                if matches == 5:
                                    break
...
print("\n=== Best Search Result ===")
print(f"Matches: {best_matches}/5")
print(best_params)
