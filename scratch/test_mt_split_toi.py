    ...
    for w in raw_words:
        ...
        for i, char in enumerate(w):
            ...
            if char in ['-', '/'] and i < len(w) - 1:
                ...
                # If followed by word chars (intra-word dot, like in email/URL)
                if w[i+1].isalnum():
                    parts.append((current, len(parts) == 0))
                    current = ""
        if current:
...
max_w = 1440
print(f"Testing max_width = {max_w} with default TOI widths + split-tokenization:")
for s in samples:
    tokens = tokenize_mt(s["text"])
    calc = calculate_lines(tokens, s["bold_words"], max_w, base_widths, 1.2, 1.2)
    print(f"{s['id']}: expected={s['expected']}, got={calc}")
