...
for matra_w in range(0, 20, 2):
    for space_w in range(15, 35, 2):
        for eng_w in range(35, 55, 2):
            for num_w in range(35, 55, 2):
                def get_w(c):
                    if c in matras: return matra_w
                    if '\u0900' <= c <= '\u097F': return 56
                    if c in '0123456789': return num_w
                    if c in ' .,-:/()!': return 25
                    if 'A' <= c <= 'Z' or 'a' <= c <= 'z': return eng_w
                    return 50
                ...
                for max_w in range(1200, 1500, 5):
                    matches = sum(1 for s in samples if calc_l(s["tokens"], max_w) == s["expected"])
                    if matches == 6:
                        print(f"PERFECT! matra={matra_w}, space={space_w}, eng={eng_w}, num={num_w}, max_w={max_w}")
                        exit(0)
