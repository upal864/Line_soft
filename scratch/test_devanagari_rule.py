...
for max_w in range(1000, 1500, 5):
    matches = sum(1 for s in samples if calc_lines(s["text"].split(' '), max_w) == s["expected"])
    if matches == len(samples):
        print(f"Perfect width found: {max_w}")
        for s in samples:
            print(f"Expected: {s['expected']}, Got: {calc_lines(s['text'].split(' '), max_w)}")
        break
