class YashobhumiCalculator {
    constructor(max_w) {
        this.TOI_COLUMN_MAX_WIDTH = max_w;
...
const samples = [
    ...
    "मुंबई स्मार्ट मीटर प्रोजेक्ट हेतू इलेक्ट्रिकल वायरमैन व সুপারवाइजर की आवश्यकता। वेतन 15000- 20000 (परक्राम्य) प्रशिक्षण उपलब्ध। प्रोत्साहन एवं करियर विकास के अवसर संपर्क: +91 8582945474",
...
for (let max_w = 1000; max_w <= 1300; max_w += 1) {
    const engine = new YashobhumiCalculator(max_w);
    let matches = 0;
    let res = [];
    samples.forEach((text, i) => {
        ...
        if (layout.linesCount === expected[i]) matches++;
        res.push(layout.linesCount);
    });
    if (matches >= 6) {
        console.log(`Max W: ${max_w}, Matches: ${matches}, Results: [${res.join(', ')}]`);
    }
    if (matches === 7) {
