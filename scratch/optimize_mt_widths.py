...
best_max_width = 1440
best_bold_mult = 1.2
best_first_bold_mult = 1.2

# We'll run a large number of random walk iterations
for i in range(100000):
    # Propose new params based on current best
    temp_widths = best_widths.copy()
    
    # perturb 1 to 3 random widths
    ...
    if temp_max_width < 1100: temp_max_width = 1100
    if temp_max_width > 1650: temp_max_width = 1650
    
    temp_bold_mult = round(best_bold_mult + random.choice([-0.1, 0, 0.1]), 1)
    if temp_bold_mult < 1.0: temp_bold_mult = 1.0
    if temp_bold_mult > 1.8: temp_bold_mult = 1.8
    
    temp_first_bold_mult = round(best_first_bold_mult + random.choice([-0.1, 0, 0.1]), 1)
    if temp_first_bold_mult < 1.0: temp_first_bold_mult = 1.0
    if temp_first_bold_mult > 2.0: temp_first_bold_mult = 2.0

    matches = 0
    results = []
    for s in samples:
        calc = calculate_lines(s["tokens"], s["bold_words"], temp_max_width, temp_widths, temp_bold_mult, temp_first_bold_mult)
        results.append(calc)
        if calc == s["expected"]:
            matches += 1
            
    if matches > best_matches or (matches == best_matches and matches == 5 and temp_max_width == 1440):
        # We prefer max_width of 1440 (same as TOI) if we get 5 matches
        best_matches = matches
        best_widths = temp_widths.copy()
        best_max_width = temp_max_width
        best_bold_mult = temp_bold_mult
        best_first_bold_mult = temp_first_bold_mult
        
        print(f"Iter {i}: matches={matches}/5, max_width={best_max_width}, bold_mult={best_bold_mult}, first_bold_mult={best_first_bold_mult}, results={results}")
        if matches == 5:
            # We found a perfect configuration!
            print("\nFOUND PERFECT MATCH CONFIGURATION!")
            print(f"Max Width: {best_max_width}")
            print(f"Bold Mult: {best_bold_mult}")
            print(f"First Bold Mult: {best_first_bold_mult}")
            print("Widths JSON:")
