...
def tokenize_mt(text):
    ...
    for w in raw_words:
        ...
        for i, char in enumerate(w):
            current += char
            # Split point: hyphen, slash, dot
            # Make sure it's not the very last character
            if char in ['-', '/', '.'] and i < len(w) - 1:
                # If dot, make sure it's not part of ellipsis ".." or "..."
                if char == '.' and w[i+1] == '.':
                    continue
                # If dot at index > 0 and char before is dot, don't split
...
if best_matches == 5:
    ...
    print(f"\nCould not find perfect match. Best matches: {best_matches}/5")
    print(best_params)
